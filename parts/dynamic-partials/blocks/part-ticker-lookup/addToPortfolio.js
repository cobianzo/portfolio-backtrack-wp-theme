...
export const setupAddToPortfolio = ( addToPortfolioButtonParent, buttonSelector ) => {

	document.querySelector(addToPortfolioButtonParent).addEventListener('click', async function(event) {
		if (event.target.matches( buttonSelector )) {
			...
			formdata.append('action', 'add_to_current_user_portfolio');
